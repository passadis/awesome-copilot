...
**`include-custom-instructions`** *(v1.0.86+)*: Set this to `true` in an agent's frontmatter to have the agent also read repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) in addition to its own persona. Without this flag, a custom agent's instructions replace the repository-wide instructions rather than layering on top of them:
