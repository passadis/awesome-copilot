...
**include-custom-instructions** *(v1.0.86+)*: Custom agents can opt into repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) that would otherwise be excluded from a custom agent's context. Set `include-custom-instructions: true` in the agent's frontmatter when the agent should still follow repo-wide conventions alongside its own persona:

```yaml
---
name: 'API Design Reviewer'
description: 'Reviews API contracts for consistency'
