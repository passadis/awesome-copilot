...
**include-custom-instructions** *(v1.0.86+)*: By default, custom agents do **not** automatically load repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) — an agent's own persona and instructions take precedence. Set `include-custom-instructions: true` to opt a specific agent into also reading those repository-level instruction files, layering your team's general conventions on top of the agent's specialized behavior:
