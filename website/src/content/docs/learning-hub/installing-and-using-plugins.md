...
> **CLI command renames (v1.0.85+)**: `copilot plugins list --kind instruction` and `copilot plugins list --kind lsp` are replaced by dedicated top-level commands: `copilot instruction list` and `copilot lsp list`. Likewise, `copilot plugins enable/disable --plugin|--mcp|--skill` is replaced by an `enable`/`disable` subcommand on each resource's own command: `copilot plugin enable <name>`, `copilot mcp enable <name>`, and `copilot skill enable <name>` (and their `disable` counterparts). `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` also gained a `--json` flag for scripting.
