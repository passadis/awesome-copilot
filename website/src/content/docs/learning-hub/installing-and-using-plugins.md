...
*(v1.0.79+)* Agent Plugins spec plugins can also ship extensions under a `com.github.copilot/extensions/` directory.
...
*(v1.0.79+)* Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to automatically update that marketplace's plugins at the start of every session, instead of requiring a manual `copilot plugin marketplace update`:
