...
**Non-interactive enable/disable commands** *(v1.0.85+)*: Enable or disable a plugin, MCP server, or skill from a script or CI job with the dedicated subcommands, which replace the old cross-kind `copilot plugins enable/disable --plugin|--mcp|--skill` syntax:
...
> **Breaking change (v1.0.85+)**: `copilot plugins list --json` now returns a flat array of plugins instead of the old `{ plugins, errors }` object — update scripts that read `.plugins` from the JSON output. `copilot plugins list` is now an alias of `copilot plugin list` and reports only plugins, not MCP servers, skills, instructions, or LSP servers. The cross-kind `--kind`, `--scope`, `--mcp`, and `--skill` flags have been removed from `copilot plugins` — use `copilot mcp` and `copilot skill` instead. Installing a skill with `copilot plugins install --skill [--scope project]` has also been replaced by `copilot skill add [--project]`.

**`copilot instruction list` and `copilot lsp list`** *(v1.0.85+)*: These new subcommands replace `copilot plugins list --kind instruction` and `--kind lsp` for listing instruction files and LSP servers from the command line. Add `--json` to `copilot plugin list`, `copilot plugin marketplace list`, or `copilot plugin marketplace browse` for machine-readable output.
