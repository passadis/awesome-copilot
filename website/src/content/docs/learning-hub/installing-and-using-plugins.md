...
> **CLI subcommands renamed (v1.0.85+)**: `copilot plugins list --kind instruction` and `--kind lsp` are replaced by dedicated `copilot instruction list` and `copilot lsp list` subcommands. Similarly, `copilot plugins enable/disable --plugin|--mcp|--skill` are replaced by `enable`/`disable` subcommands directly on `copilot plugin`, `copilot mcp`, and `copilot skill` (for example, `copilot plugin enable my-plugin`). `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` also gained a `--json` flag for scripting.

> **`copilot mcp list` shows built-in GitHub MCP server (v1.0.87+)**: When you are signed in, `copilot mcp list` and `copilot mcp get` now report the built-in `github-mcp-server`, matching what was previously only visible in the interactive `/mcp` view.
