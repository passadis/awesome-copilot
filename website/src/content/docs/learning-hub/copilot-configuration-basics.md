...
**Vim mode** *(v1.0.84+)*: Vim mode is available to everyone. Turn it on with `/vim` or by setting `editorMode` to `vim` in `/settings` for modal editing (normal/insert modes, familiar motions and commands) in the composer. The current editing mode is shown while you type.

**Session and memory import** *(v1.0.84+)*: Use `copilot session import` and `copilot memory import` to bring sessions and memory data into the CLI using a semantic JSONL interchange format — useful for migrating history between machines or restoring from a backup export.
