...
**Vim mode** *(v1.0.85+)*: Vim mode is available to everyone. Turn it on with `/vim` or by setting `editorMode` to `vim` in `/settings` to get modal editing (normal/insert modes, `hjkl` navigation, and common Vim motions) in the composer. The current mode is shown while you type, so you always know whether you're in normal or insert mode.

**`/config`** *(v1.0.85+)*: Opens a sidebar configuration screen inside the CLI, giving you a menu-driven way to browse and change settings without hand-editing `config.json`.
