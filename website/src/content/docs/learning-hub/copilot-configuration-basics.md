...
**Streamer mode** *(v1.0.85+)*: Toggle streamer mode to mask internal model names in `/model`, the footer, and startup diagnostics — useful when screen-sharing or streaming a session so viewers don't see which underlying model you're using. Toggling it takes effect without restarting model initialization.
...
**`/config`** *(v1.0.85+)*: Opens a sidebar configuration screen inside the CLI, giving you a persistent panel for reviewing and adjusting configuration alongside your active conversation, rather than a modal dialog:

```
/config
```

**Vim mode** *(v1.0.85+, available to everyone)*: Turn on modal editing in the composer with `/vim`, or set `editorMode` to `vim` in your settings to enable it by default. The current mode (insert/normal) is shown while you type, matching the familiar Vim editing model for those who prefer it:

```
/vim
```

**Concise transcript view** *(v1.0.85+)*: Set `transcriptView` to `"concise"` in `/settings` to group tool activity into expandable work summaries instead of showing every tool call inline. This keeps the timeline shorter and easier to scan during long agentic sessions, while still letting you expand a summary to see the underlying tool calls.
...
**Network host allow/deny rules** *(v1.0.85+)*: `/sandbox` now supports per-host allow and deny rules for network access, layered on top of your configured upstream proxy rather than replacing it. This lets you permit or block specific hosts (for example, allow an internal API host while keeping the general proxy policy in place) without having to reconfigure your proxy setup.
