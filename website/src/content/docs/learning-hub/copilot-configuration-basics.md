...
**Vim mode** *(v1.0.85+)*: Modal (vim-style) editing in the composer is now available to everyone. Turn it on with the `/vim` slash command, or set `editorMode` to `vim` in your settings. The current mode is shown while you type, so you always know whether you're in normal or insert mode.

**`/config` sidebar** *(v1.0.85+)*: The `/config` command opens a sidebar configuration screen inside the CLI, giving you a persistent panel for browsing and changing settings without leaving your session — a lighter-weight alternative to the full-screen `/settings` dialog for quick adjustments.

**Concise transcript view** *(v1.0.85+)*: Set `transcriptView` to `"concise"` in your settings to group tool activity into expandable work summaries instead of a long scrolling list of individual tool calls. This keeps the conversation view focused on the model's responses while still letting you expand a summary to see the underlying tool calls when you need the detail.
...
**Network host allow/deny rules** *(v1.0.85+)*: `/sandbox` now supports per-host network allow and deny rules, letting you permit or block specific hosts without replacing your configured upstream proxy. This gives finer-grained control than the previous all-or-nothing network toggle — for example, you can allow a specific internal API host while keeping the rest of the network restricted through your proxy.
