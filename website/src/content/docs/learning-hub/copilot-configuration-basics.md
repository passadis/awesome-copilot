...
| `editorMode` | Set to `vim` for modal editing in the composer input (v1.0.84+) |

> **Vim mode (v1.0.84+)**: Toggle modal (Vim-style) editing in the composer with `/vim`, or set `editorMode` to `vim` in your config to enable it by default. The current mode (insert/normal) is shown while you type, the same way it would in a terminal Vim session.

> **Disabling the Windows taskbar status (v1.0.84+)**: If you don't want running sessions to appear in the Windows 11 taskbar, run `/settings taskbarPresence false`. Sessions managed by the loader restart immediately to pick up the change; standalone sessions require a manual restart.
