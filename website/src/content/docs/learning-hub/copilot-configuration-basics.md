...
**Auto routing tier defaults** *(v1.0.87+)*: Organizations can now set a **default routing tier** for users on the **Auto** model — Efficiency, Balance, or Intelligence — via a managed policy, with a **strict** flag that prevents individual users from overriding it. Without a strict policy, users can still adjust their own default tier in `/settings`. This gives admins a way to control cost/quality tradeoffs for teams that rely on Auto mode without disabling it outright.
...
**`/config` sidebar** *(v1.0.85+)*: Run `/config` to open a dedicated sidebar configuration screen inside the CLI, separate from the main `/settings` dialog. It's a quicker way to jump into configuration while keeping the main conversation panel visible alongside it.

**Vim mode** *(v1.0.85+)*: Modal editing in the composer is now available to everyone. Turn it on with `/vim` or by setting `editorMode` to `vim` in your settings. The current mode (insert/normal) is shown while you type, matching familiar Vim keybindings for navigating and editing your prompt text.

**Session and memory import** *(v1.0.85+)*: Use the new session and memory import commands to bring conversations and memory into the CLI using the semantic JSONL interchange format — useful for migrating history from another tool or restoring a shared context bundle across machines.
...
**Fork during an active turn** *(v1.0.88+)*: You can now run `/fork` while the agent is still working on a response, letting you branch off to try a different direction without waiting for the current turn to finish.
...
**`worktreePathTemplate` setting** *(v1.0.87+)*: Controls where `/worktree`, `/move`, `/new`, and `--worktree` create worktrees, using a path template with `{repoPath}`, `{repo}`, `{branch}`, and `{branchSlug}` placeholders — for example, `~/src/worktrees/{repo}/{branch}`. Leave it unset to keep the current default layout (`<repo>.worktrees/`, with slashes in the branch name flattened to dashes).
...
> **Network host allow/deny rules (v1.0.85+)**: Use `/sandbox` to add specific network host allow/deny rules without replacing your configured upstream proxy — useful when you want the sandbox to reach a small set of additional hosts (like an internal package registry) while keeping your organization's proxy in place for everything else.
